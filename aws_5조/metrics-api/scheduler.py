...
QUERY = '100 - avg by(instance)(rate(node_cpu_seconds_total{mode="idle"}[15s])) * 100'
...
def generate_dynamic_config():
    ...
    # best_node 형식 예시: "211.183.3.201:9100"
    if best_node and ":" in best_node:
        best_ip = best_node.split(":")[0]
    else:
        best_ip = best_node or "localhost"

    print(f"[DEBUG] Prometheus best node: {best_node}")
    print(f"[DEBUG] Extracted IP: {best_ip}")
